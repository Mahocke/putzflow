...
// Was der Knopf tut, hängt an der Auswahl daneben — nicht am Zustand des Termins:
//   niemand gewählt     → Zuteilung entfernen (die Kraft bekommt eine Kalender-Absage)
//   andere Person       → Umteilen (dieselbe Absage an die alte)
//   dieselbe, angefragt → Erinnern (nachfassen, ohne den Zustand anzufassen)
//   dieselbe, zugesagt  → KEIN Knopf
//
// ⚠️ Auf einer ZUGESAGTEN Karte stand hier zweimal etwas Falsches. Erst „Erneut
// anfragen", das den Termin still zurücksetzte (siehe /assign in server.js).
// Dann „Nochmal senden", das zwar nichts mehr kaputt machte, aber keinen Anlass
// hatte: Wer zugesagt hat, hat die Anfrage gelesen und den Termin im Kalender —
// es gibt nichts zu senden. Ein Knopf ohne Anwendungsfall wird trotzdem gedrückt,
// und dann wundert sich eine Kraft über eine Anfrage zu einem Termin, den sie
// längst bestätigt hat. **Kein Knopf ist die richtige Beschriftung für nichts zu tun.**
// Er kommt zurück, sobald die Auswahl sich ändert — dann gibt es wieder etwas zu tun.
//
// ⚠️ Beim ANGEFRAGTEN Termin bleibt er, heißt aber „Erinnern". Dort ist der Fall
// echt: Sie hat noch nicht geantwortet, und eine automatische Erinnerung gibt es
// nicht. „Nochmal senden" beschrieb die Technik (eine Mail geht raus), „Erinnern"
// beschreibt den Anlass — und nur den kennt die Chefin, wenn sie draufdrückt.
function sendeKnopf(j, gewaehlt) {
  ...
  if (j.assigned_user_id === g) {
    return j.confirmed ? { text: '', klasse: 'ghost', aus: true, weg: true }
                       : { text: 'Erinnern', klasse: 'ghost', aus: false };
  }
...
// Eine Frage mit mehreren Auswegen. `confirm()` kann nur ja/nein, und genau daran
// scheiterte „Absagen": Dahinter stecken ZWEI Vorgänge, die nichts miteinander zu
// tun haben — die Kraft fällt aus (die Reinigung muss trotzdem gemacht werden) oder
// es ist gar keine Reinigung nötig (der Gast war nie da). Wer beides unter einen
// Knopf legt, lässt die Chefin raten, was gleich passiert.
// Liefert den Schlüssel des gewählten Weges oder null bei Abbruch.
function frage({ titel, text, wege }) {
  return new Promise(fertig => {
    const d = document.createElement('dialog');
    d.className = 'frage';
    d.innerHTML = `
      <h3>${titel}</h3>
      ${text ? `<p class="small muted" style="margin:.2rem 0 .6rem">${text}</p>` : ''}
      ${wege.map(w => `<button class="weg ${w.klasse || 'ghost'}" data-weg="${w.key}">
           <b>${w.titel}</b><span>${w.warum}</span></button>`).join('')}
      <button class="ghost weg" data-weg="" style="margin-top:1rem">Abbrechen</button>`;
    document.body.appendChild(d);
    d.addEventListener('click', ev => {
      const k = ev.target.closest('button[data-weg]');
      if (!k) return;
      d.close(k.dataset.weg);
    });
    // Esc schließt <dialog> von selbst — dann steht returnValue auf '' und das
    // ist derselbe Ausgang wie „Abbrechen". Kein Sonderfall nötig.
    d.addEventListener('close', () => { const v = d.returnValue; d.remove(); fertig(v || null); });
    d.showModal();
  });
}
...
function toast(msg, art) {
  const t = $('#toast');
  t.textContent = msg;
  // ⚠️ Eine Warnung braucht länger als eine Bestätigung. 2,4 Sekunden reichen für
  // „Gespeichert"; wer erfahren soll, dass eine Terminanfrage NICHT hinausging,
  // liest den Satz in der Zeit nicht zu Ende.
  t.style.background = art === 'warn' ? 'var(--bad)' : '';
  t.classList.add('on');
  clearTimeout(toast._weg);
  toast._weg = setTimeout(() => t.classList.remove('on'), art === 'warn' ? 7000 : 2400);
}
...
function tabsHtml() {
  const t = [['jobs', 'Termine'], ['month', 'Monat'], ['team', 'Team'], ['units', 'Unterkünfte'],
             ['urlaub', 'Urlaub'], ['sheet', 'Stundenzettel'], ['konto', 'Konto']];
...
async function renderJobs() {
  ...
  // Abgesagte ebenso: Sie sind erledigt in dem Sinn, dass niemand mehr etwas tun
  // muss, sollen aber sichtbar bleiben. Eine Reinigung, die lautlos verschwindet,
  // fehlt am Monatsende und niemand weiß, ob sie vergessen oder abgesagt wurde.
  const erledigtOderWeg = j => j.status === 'done' || j.status === 'skipped';
  const sorted = [...data.jobs].sort((a, b) =>
    erledigtOderWeg(a) - erledigtOderWeg(b) || a.due_date.localeCompare(b.due_date));

  const cards = sorted.map(j => {
    // ⚠️ „keine Reinigung nötig", NICHT „abgesagt". Beide Wörter stünden sonst in
    // derselben Liste für zwei verschiedene Ausgänge: `declined_at` heißt, die KRAFT
    // hat abgesagt — dann fällt der Termin auf offen zurück und der Rundruf läuft an,
    // es ist also weiter Arbeit da.
    // ⚠️ Und NICHT „entfällt" (so hieß es einen halben Tag lang). Das Wort las sich,
    // als wäre der Termin weg — ist er aber nicht: Er bleibt stehen, bleibt an die
    // Smoobu-Buchung gebunden und wandert mit, wenn die Buchung verschoben wird. Weg
    // ist nur die Arbeit. Der Zustand heißt deshalb nach dem, was zutrifft, und nicht
    // nach dem, was man beim Klicken gerade tut. Etwas länger als die übrigen
    // Abzeichen — dafür muss es niemand erklären.
    const badge = j.status === 'skipped' ? '<span class="pill">keine Reinigung nötig</span>'
      : j.status === 'done' ? '<span class="pill ok">erledigt</span>'
    ...
    // Bei abgesagten dasselbe, nur mit einem Weg zurück: Ein Fehlklick darf keinen
    // Termin endgültig vernichten. Kein Auswahlfeld — zugeteilt wird erst wieder,
    // wenn der Termin steht.
    const controls = j.status === 'skipped'
      ? `<div class="row" style="margin-top:.6rem">
           <button data-einplanen="${j.id}" class="ghost">Doch einplanen</button>
           <span class="small muted">Kommt als offener Termin zurück und muss neu angefragt werden.</span>
         </div>`
      : j.status === 'done'
      ...
      : `<div class="row" style="margin-top:.6rem">
           ...
           ${(k => `<button data-send="${j.id}" class="${k.klasse}"${k.aus ? ' disabled' : ''}${k.weg ? ' hidden' : ''}>${k.text}</button>`)(sendeKnopf(j, j.assigned_user_id))}
           ...
           <button data-absagen="${j.id}" class="ghost" title="Die Reinigung fällt weg — nicht zu verwechseln mit dem Wechsel der Kraft">Absagen</button>
         </div>
         ${budgetHint(j)}
...
async function renderMonth() {
  ...
  const proTag = {};
  // ⚠️ Abgesagte Reinigungen bekommen KEINEN Punkt. Der Kalender beantwortet eine
  // einzige Frage — wann ist viel los? — und dafür zählt nur, was zu tun ist. Ein
  // fünfter Farbton hätte die vier Farben der Legende auf der Startseite gebrochen
  // und die Ansicht für einen Termin unruhiger gemacht, der gerade niemanden mehr
  // beschäftigt. In der Terminliste steht er weiterhin, dort ist er nachvollziehbar.
  for (const j of data.jobs) if (j.status !== 'skipped') (proTag[j.due_date] ||= []).push(j);
  ...
  for (let d = 1; d <= tageImMonat; d++) {
    ...
    // ⚠️ EIN Punkt je Reinigung, nicht je Zustand (geändert 27.07.2026). Vorher
    // wurden gleiche Zustände zu einem Punkt zusammengefasst — drei erledigte
    // Reinigungen an einem Freitag sahen dann aus wie eine. Damit war der
    // Monatskalender an genau der Frage blind, für die es ihn gibt: Wann ist
    // viel los? Nebenbei versprach die Startseite schon immer „ein Farbpunkt je
    // Reinigung". Nach Zustand sortiert, damit die Farben je Tag beieinander
    // stehen; die vollständige Liste steht weiterhin im Titel der Zelle.
    const rang = Object.keys(DAY_STATUS);
    const punkte = jobs
      .map(j => jobStatusKey(j))
      .sort((a, b) => rang.indexOf(a) - rang.indexOf(b))
      .map(k => `<span class="cal-punkt" style="background:${DAY_STATUS[k].farbe}"></span>`)
      .join('');
...
async function renderTeam() {
  ...
  // ⚠️ Nachrichten, die nicht angekommen sind. Bis zum 21.08.2026 landeten sie in
  // `notify_log` und wurden dort NIE wieder gelesen — beim ersten echten Kunden
  // vier Terminanfragen an einem Vormittag. Der Hinweis steht im Reiter „Team",
  // weil dort die Ursache liegt und behoben wird, nicht in einem eigenen Bereich,
  // den niemand aufsucht.
  const zustellung = await api('/api/zustellprobleme').catch(() => ({ anzahl: 0, probleme: [] }));
  const zustellHinweis = zustellung.anzahl ? `<div class="card" style="border-left:3px solid var(--bad)">
    <h2 style="margin:0 0 .3rem">${zustellung.anzahl === 1 ? 'Eine Nachricht kam nicht an' : `${zustellung.anzahl} Nachrichten kamen nicht an`}</h2>
    <p class="small muted" style="margin:0 0 .5rem">In den letzten 14 Tagen. Die betroffene
    Kraft weiß also nichts von ihrem Termin — bitte kurz nachfassen.</p>
    <ul class="small muted" style="margin:0;padding-left:1.1rem">
      ${zustellung.probleme.slice(0, 5).map(pr => `<li>${new Date(pr.created_at.replace(' ', 'T') + 'Z').toLocaleString('de-DE')}
        — ${pr.subject || 'Nachricht'} <span style="color:var(--bad)">(${pr.error || 'nicht zugestellt'})</span></li>`).join('')}
      ${zustellung.anzahl > 5 ? `<li>… und ${zustellung.anzahl - 5} weitere</li>` : ''}
    </ul>
  </div>` : '';
  ...
  const karte = u => `<div class="card">
    <div class="row spread">
      <h2>${u.name}${u.silent ? ' <span class="pill">still</span>' : ''}${u.active ? '' : ' <span class="pill bad">stillgelegt</span>'}${
        u.unerreichbar ? ' <span class="pill bad">nicht erreichbar</span>' : ''}</h2>
      ${budget(u)}
    </div>
    ...
    ${u.unerreichbar ? `<p class="small" style="color:var(--bad);margin:.4rem 0 0">
      ⚠️ Ohne E-Mail-Adresse bekommt ${u.name} <strong>keine Terminanfragen</strong> — Zuteilungen
      laufen dann ins Leere. Entweder eine Adresse eintragen, oder „still" setzen und den Link
      unten selbst weitergeben (dann erwartet Putzflow auch keine Zusage mehr).</p>` : ''}
  ...
  $('#content').innerHTML = tabsHtml() + zustellHinweis +
  ...
  $('#content').innerHTML = tabsHtml() + `
    <div class="card">
      ...
      ${sm.verbunden ? `
        ...
        nichts in Ihr Smoobu-Konto zurück. Auch ohne die folgende Einstellung wird
        alle 15 Minuten von selbst abgeglichen.</p>
        ${sm.webhook_url ? `
        <h3 style="margin:1rem 0 .2rem;font-size:1rem">Sofort statt alle 15 Minuten</h3>
        <p class="small muted" style="margin:0 0 .4rem">Tragen Sie diese Adresse in Smoobu
        unter <strong>Einstellungen → Erweitert → Webhooks</strong> ein. Smoobu meldet
        Putzflow dann jede neue, geänderte oder stornierte Buchung sofort — bei einer
        Spontanbuchung für morgen ist das der Unterschied zwischen einer informierten
        Reinigungskraft und einer verpassten Reinigung.</p>
        <div class="row" style="flex-wrap:wrap">
          <input id="wh-url" readonly value="${sm.webhook_url}"
                 style="min-width:20rem;flex:1;font-family:monospace;font-size:.8rem">
          <button class="ghost" id="wh-kopieren">Kopieren</button>
        </div>
        <p class="small muted" style="margin:.4rem 0 0">Die Adresse enthält ein Geheimnis —
        behandeln Sie sie wie ein Passwort. Ist sie versehentlich weitergegeben worden:
        <button class="ghost small" id="wh-neu">neue Adresse erzeugen</button>
        (die alte gilt dann sofort nicht mehr und muss in Smoobu ersetzt werden).</p>` : ''}`
...
// --- Urlaub ---------------------------------------------------------------
// Zwei Fragen auf einer Seite, in dieser Reihenfolge: Was liegt zur Entscheidung an?
// Und wie stehen die Konten? Offene Anträge zuerst — alles andere kann warten, ein
// unbeantworteter Antrag nicht.
async function renderUrlaub() {
  const d = await api('/api/urlaub');
  const offen = d.antraege.filter(a => a.status === 'pending');
  const rest = d.antraege.filter(a => a.status !== 'pending');
  const zeitraum = a => a.start_date === a.end_date
    ? fmtTag(a.start_date) : `${fmtTag(a.start_date)} – ${fmtTag(a.end_date)}`;

  const antragKarte = a => `<div class="card">
    <div class="row spread">
      <h2>${a.user_name}</h2>
      <span class="pill warn">wartet auf Antwort</span>
    </div>
    <p style="margin:.3rem 0">${zeitraum(a)}</p>
    ${a.note ? `<p class="small muted">Notiz: ${a.note}</p>` : ''}
    <div class="row" style="margin-top:.5rem;flex-wrap:wrap">
      <button class="primary" data-url-ja="${a.id}">Genehmigen</button>
      <button class="ghost" data-url-nein="${a.id}">Ablehnen</button>
    </div>
  </div>`;

  // ⚠️ Der Kontostand steht bei der PERSON, nicht beim Antrag: Beim Entscheiden ist
  // die Frage „hat sie die Tage überhaupt noch?", und die beantwortet keine Zeile,
  // die nur den beantragten Zeitraum nennt.
  const personKarte = p => {
    const k = p.konto;
    return `<div class="card">
      <div class="row spread">
        <h2>${p.name}</h2>
        ${k ? `<span class="pill ${k.rest < 0 ? 'bad' : k.rest === 0 ? 'warn' : 'ok'}">${k.rest} von ${k.anspruch} frei</span>`
            : '<span class="muted small">kein Urlaubskonto</span>'}
      </div>
      ${k && k.uebertrag_rest ? `<p class="small muted" style="margin:.2rem 0">Davon ${k.uebertrag_rest} Tage
        Übertrag aus ${k.jahr - 1} — verfallen am ${fmtTag(k.uebertrag_verfaellt)}.</p>` : ''}
      ${k && k.uebertrag_unbekannt ? `<p class="small muted" style="margin:.2rem 0">Der Anspruch
        besteht laut Eintrag schon länger, als Putzflow das Konto führt — ein Resturlaub aus
        der Zeit davor ist hier <strong>nicht</strong> enthalten. Putzflow erfindet ihn nicht;
        wenn es welchen gibt, bitte von Hand berücksichtigen.</p>` : ''}
      ${k && k.rest < 0 ? `<p class="small" style="color:var(--warn);margin:.2rem 0">Überzogen.
        Putzflow blockt das nicht — Urlaub im Voraus zu gewähren ist erlaubt —, aber es sollte
        bewusst geschehen.</p>` : ''}
      <form class="row" data-urlaub-person="${p.id}" style="margin-top:.4rem;flex-wrap:wrap">
        <label class="small muted">Urlaubstage/Jahr
          <input name="vacation_days" type="text" inputmode="decimal" value="${p.vacation_days ?? ''}"
                 placeholder="leer = kein Konto" style="width:8rem"></label>
        <label class="small muted">Anspruch ab
          <input name="vacation_start" type="date" value="${p.vacation_start || ''}" style="width:9.5rem"></label>
        <label class="small muted row" style="gap:.3rem"><input type="checkbox" name="vacation_paid"${p.vacation_paid ? ' checked' : ''}> bezahlt</label>
        <button class="ghost" type="submit">Speichern</button>
      </form>
    </div>`;
  };

  const erledigt = a => `<div class="row spread" style="padding:.35rem 0;border-top:1px solid var(--line)">
    <div>
      <div>${a.user_name} · ${zeitraum(a)}</div>
      <div class="small muted">${a.days ? a.days + (a.days === 1 ? ' Tag' : ' Tage') + ' · ' : ''}${
        { approved: 'genehmigt', declined: 'abgelehnt', cancelled: 'zurückgezogen' }[a.status] || a.status}${
        a.paid === 0 && a.status === 'approved' ? ' · unbezahlt' : ''}</div>
    </div>
    <div style="text-align:right">
      ${a.status === 'approved' && a.pay_cents ? `<div><strong>${euro(a.pay_cents)}</strong></div>` : ''}
      ${a.status === 'approved' ? `<button class="ghost small" data-url-weg="${a.id}">zurücknehmen</button>` : ''}
    </div>
  </div>`;

  const mitKonto = d.personen.filter(p => p.konto);

  $('#content').innerHTML = tabsHtml()
    + (offen.length
        ? offen.map(antragKarte).join('')
        : '<div class="card muted">Keine offenen Urlaubsanträge.</div>')
    + `<div class="card">
        <h2>Urlaub eintragen</h2>
        <p class="small muted">Was Sie hier eintragen, ist sofort genehmigt — es gibt
        niemanden mehr, der es entscheiden müsste.</p>
        <form class="row" id="urlaubneu" style="margin-top:.5rem;flex-wrap:wrap">
          <select name="user_id" required>
            <option value="">Person wählen …</option>
            ${mitKonto.map(p => `<option value="${p.id}">${p.name}</option>`).join('')}
          </select>
          <label class="small muted">Von <input name="von" type="date" required></label>
          <label class="small muted">Bis <input name="bis" type="date"></label>
          <button class="primary" type="submit">Eintragen</button>
        </form>
        ${mitKonto.length ? '' : `<p class="small" style="color:var(--warn);margin:.5rem 0 0">
          Für niemanden sind Urlaubstage hinterlegt — bitte unten je Person eintragen.</p>`}
      </div>`
    + d.personen.map(personKarte).join('')
    + (rest.length ? `<div class="card"><h2>Verlauf</h2>${rest.map(erledigt).join('')}</div>` : '')
    + `<div class="card">
        <h2>Zählweise</h2>
        <form class="row" id="urlaubwerktage" style="flex-wrap:wrap">
          <label class="small muted">Ein Urlaubstag ist ein …
            <select name="urlaub_werktage">
              <option value="6"${d.werktage === 6 ? ' selected' : ''}>Werktag (Mo–Sa)</option>
              <option value="5"${d.werktage === 5 ? ' selected' : ''}>Arbeitstag (Mo–Fr)</option>
            </select></label>
          <button class="ghost" type="submit">Speichern</button>
        </form>
        <p class="small muted" style="margin:.5rem 0 0"><strong>Werktage (Mo–Sa)</strong> ist der
        gesetzliche Begriff (§ 3 Abs. 2 BUrlG) und die Voreinstellung: Eine freie Woche kostet
        dann 6 Tage, 30 Urlaubstage sind 5 Wochen. Steht in Ihren Verträgen „Arbeitstage",
        stellen Sie auf 5 um — 25 Arbeitstage sind dieselben 5 Wochen.</p>
        <p class="small muted" style="margin:.4rem 0 0">Sonn- und Feiertage gehen nie vom
        Urlaub ab. Die Umstellung wirkt nur auf künftige Genehmigungen; schon genehmigter
        Urlaub behält seine Zählung, damit sich abgezeichnete Stundenzettel nicht
        rückwirkend ändern.</p>
      </div>`;
}
...
async function renderTab() {
  try {
    ...
    else if (tab === 'urlaub') await renderUrlaub();
...
document.addEventListener('click', async ev => {
  ...
  const urlJa = ev.target.closest('button[data-url-ja]');
  if (urlJa) {
    try {
      const r = await api(`/api/urlaub/${urlJa.dataset.urlJa}/entscheiden`, { entscheidung: 'approve' });
      // Der eingefrorene Tagessatz gehört in die Rückmeldung: Ist er 0, weil keine
      // dreizehn Wochen Verdienst vorliegen, muss die Chefin es JETZT erfahren —
      // nicht erst, wenn der Stundenzettel bei der Lohnbuchhaltung liegt.
      toast(r.pro_tag_cents
        ? `Genehmigt — ${r.tage} ${r.tage === 1 ? 'Tag' : 'Tage'} à ${euro(r.pro_tag_cents)}.`
        : `Genehmigt — ${r.tage} ${r.tage === 1 ? 'Tag' : 'Tage'}. Achtung: kein Urlaubsentgelt berechenbar (keine 13 Wochen Verdienst).`);
      await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }

  const urlNein = ev.target.closest('button[data-url-nein]');
  if (urlNein) {
    // ⚠️ Nach einem Grund wird gefragt, nicht bloß bestätigt. Eine Ablehnung ohne
    // Begründung ist für die Kraft nicht von Willkür zu unterscheiden — und die
    // Nachfrage kommt dann ohnehin, nur mündlich und später.
    const grund = prompt('Warum wird der Urlaub abgelehnt? (steht so in der Mail an die Kraft)');
    if (grund === null) return;
    try {
      await api(`/api/urlaub/${urlNein.dataset.urlNein}/entscheiden`, { entscheidung: 'decline', grund });
      toast('Abgelehnt — die Kraft hat Bescheid bekommen.');
      await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }

  const urlWeg = ev.target.closest('button[data-url-weg]');
  if (urlWeg) {
    if (!confirm('Diesen genehmigten Urlaub zurücknehmen? Die Kraft bekommt eine Nachricht.')) return;
    try {
      await api(`/api/urlaub/${urlWeg.dataset.urlWeg}`, null, 'DELETE');
      toast('Zurückgenommen.'); await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }
  ...
  if (send) {
    ...
    try {
      ...
      // ⚠️ Vorher stand hier bei einem Fehlschlag schlicht „Zugeteilt" — der
      // Unterschied zu „Angefragt" war das EINZIGE Zeichen, dass niemand
      // benachrichtigt wurde. Beim ersten echten Kunden ging das viermal
      // unter. Ein Fehlschlag muss sich wie einer anfühlen.
      if (r.zustellung && !r.zustellung.ok) {
        toast(`Zugeteilt, aber NICHT benachrichtigt: ${r.zustellung.grund}`, 'warn');
      } else {
  ...
  // Absagen — und dahinter die Verzweigung, um die es eigentlich geht.
  //
  // ⚠️ „Abgesagt" heißt im Alltag zweierlei, und die beiden Fälle enden entgegen-
  // gesetzt:
  //   Die Kraft fällt aus (krank)      → die Reinigung MUSS trotzdem gemacht werden
  //   Keine Reinigung nötig (Gast weg) → es ist nichts mehr zu tun
  // Wer das unter einen Knopf legt, macht aus einer Entscheidung ein Ratespiel.
  // Deshalb fragt der Knopf zurück — aber nur, wenn es überhaupt jemanden
  // freizugeben gibt. Ist niemand zugeteilt, kann er nur eines heißen.
  //
  // Die Rückfrage NENNT die Kraft: Bei einer zugesagten Reinigung geht eine
  // Kalender-Absage hinaus, die den Termin aus ihrem Kalender löscht. Das soll
  // eine Entscheidung sein, kein Versehen.
  const ab = ev.target.closest('button[data-absagen]');
  if (ab) {
    const id = ab.dataset.absagen;
    const job = letzteJobs.find(x => String(x.id) === String(id));
    const wer = job && job.assigned_user_id
      ? (users.find(u => u.id === job.assigned_user_id) || {}).name : null;
    const wann = job ? fmtDate(job.due_date) : 'diesem Tag';

    let weg = 'keine';
    if (wer) {
      weg = await frage({
        titel: `Reinigung am ${wann}`,
        text: `${wer} hat den Termin${job.confirmed ? ' zugesagt' : ''}. Was ist der Fall?`,
        wege: [
          { key: 'neu', titel: `${wer} fällt aus`,
            warum: 'Die Reinigung bleibt bestehen und wird sofort allen anderen '
                 + 'angeboten — wer zuerst zusagt, bekommt sie.' },
          { key: 'keine', titel: 'Keine Reinigung nötig',
            warum: 'Zum Beispiel: der Gast war gar nicht da. Der Termin bleibt stehen '
                 + 'und wird als „keine Reinigung nötig" geführt. Zurück geht es über '
                 + '„Doch einplanen".' },
        ],
      });
      if (!weg) return;
    } else if (!confirm(`Für die Reinigung am ${wann} ist keine Reinigung nötig?\n\n`
                      + 'Der Termin bleibt stehen und wird als „keine Reinigung nötig" geführt. '
                      + 'Zurücknehmen geht über „Doch einplanen".')) return;

    ab.disabled = true;
    try {
      const r = await api(`/api/jobs/${id}/${weg === 'neu' ? 'neu-verteilen' : 'absagen'}`, {});
      toast(weg === 'neu'
        ? (r.gefragt?.length ? `Neu ausgeschrieben — gefragt: ${r.gefragt.join(', ')}`
                             : 'Zuteilung frei — es konnte NIEMAND gefragt werden')
        : 'Vermerkt: keine Reinigung nötig.');
      // Der Termin fällt aus Lohn und Verdienstgrenze heraus — Team neu laden,
      // sonst zeigen die Auswahlfelder veraltete Restbeträge.
      users = (await api('/api/users')).users;
      await renderTab();
    } catch (e) { toast(e.message); } finally { ab.disabled = false; }
    return;
  }

  const wieder = ev.target.closest('button[data-einplanen]');
  if (wieder) {
    const id = wieder.dataset.einplanen;
    wieder.disabled = true;
    try {
      await api(`/api/jobs/${id}/einplanen`, {});
      toast('Wieder eingeplant — jetzt offen.');
      users = (await api('/api/users')).users;
      await renderTab();
    } catch (e) { toast(e.message); } finally { wieder.disabled = false; }
    return;
  }
  ...
  const whKop = ev.target.closest('#wh-kopieren');
  if (whKop) {
    const feld = $('#wh-url');
    // ⚠️ Erst markieren, dann in die Zwischenablage: navigator.clipboard gibt es
    // nur über HTTPS, und wer die Anwendung im Heimnetz über http betreibt, stünde
    // sonst vor einem Knopf, der nichts tut. Markiert ist die Adresse in jedem Fall.
    feld.select(); feld.setSelectionRange(0, 99999);
    try { await navigator.clipboard.writeText(feld.value); toast('Adresse kopiert'); }
    catch { toast('Adresse ist markiert — jetzt kopieren'); }
    return;
  }

  const whNeu = ev.target.closest('#wh-neu');
  if (whNeu) {
    if (!confirm('Neue Adresse erzeugen? Die alte gilt sofort nicht mehr — bis Sie die '
               + 'neue in Smoobu eingetragen haben, wird nur alle 15 Minuten abgeglichen.')) return;
    try { await api('/api/smoobu/webhook-neu', {}); await renderTab(); toast('Neue Adresse erzeugt'); }
    catch (e) { toast(e.message); }
    return;
  }
...
document.addEventListener('change', ev => {
  ...
  if (zut) {
    ...
    if (j && btn) {
      ...
      // Der Knopf verschwindet auf einer zugesagten Karte — und muss zurückkommen,
      // sobald wieder etwas zu tun ist. Deshalb versteckt, nicht entfernt.
      btn.hidden = !!k.weg;
    }
  }
});
...
document.addEventListener('submit', async ev => {
  const urlaubNeu = ev.target.closest('#urlaubneu');
  if (urlaubNeu) {
    ev.preventDefault();
    const fd = new FormData(urlaubNeu);
    try {
      const r = await api('/api/urlaub', {
        user_id: fd.get('user_id'), von: fd.get('von'), bis: fd.get('bis') || fd.get('von'),
      });
      toast(`Eingetragen — ${r.tage} ${r.tage === 1 ? 'Tag' : 'Tage'}.`);
      await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }

  const werktage = ev.target.closest('#urlaubwerktage');
  if (werktage) {
    ev.preventDefault();
    try {
      await api('/api/tenant', { urlaub_werktage: new FormData(werktage).get('urlaub_werktage') }, 'PATCH');
      toast('Zählweise gespeichert'); await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }

  const urlaubPerson = ev.target.closest('form[data-urlaub-person]');
  if (urlaubPerson) {
    ev.preventDefault();
    const fd = new FormData(urlaubPerson);
    try {
      await api(`/api/users/${urlaubPerson.dataset.urlaubPerson}`, {
        vacation_days: fd.get('vacation_days'),
        vacation_start: fd.get('vacation_start'),
        vacation_paid: !!fd.get('vacation_paid'),
      }, 'PATCH');
      toast('Gespeichert'); await renderUrlaub();
    } catch (e) { toast(e.message); }
    return;
  }
