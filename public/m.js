...
function render() {
  ...
  $('#content').innerHTML = `
    ...
    ${urlaubKarte(state.urlaub)}
    ${auslagenKarte(ts.auslagen)}
    ${upcoming.length ? upcoming.map(jobCard).join('') : '<div class="card muted">Zurzeit keine Termine.</div>'}
  `;
}

// Urlaub. ⚠️ Ohne hinterlegtes Konto erscheint der ganze Abschnitt NICHT — „0 von 0
// Tagen frei" wäre eine Aussage über einen Anspruch, den niemand geprüft hat, und
// erzeugt genau den Anruf, den die Seite ersparen soll.
function urlaubKarte(u) {
  if (!u || !u.konto) return '';
  const k = u.konto;
  const zustand = { pending: 'wartet auf Antwort', approved: 'genehmigt',
                    declined: 'abgelehnt', cancelled: 'zurückgezogen' };

  const zeile = a => {
    const zeitraum = a.start_date === a.end_date
      ? fmtTag(a.start_date)
      : `${fmtTag(a.start_date)} – ${fmtTag(a.end_date)}`;
    // Zurückziehen geht, solange der Urlaub noch nicht begonnen hat. Danach ist es
    // eine Korrektur an einer laufenden Abrechnung und gehört in die Verwaltung.
    const wegKnopf = ['pending', 'approved'].includes(a.status) && a.start_date > state.today
      ? `<button class="ghost small" data-url-weg="${a.id}">zurückziehen</button>` : '';
    return `<div class="row spread" style="padding:.35rem 0;border-top:1px solid var(--line)">
      <div>
        <div>${zeitraum}</div>
        <div class="small muted">${a.days ? a.days + (a.days === 1 ? ' Tag' : ' Tage') + ' · ' : ''}${zustand[a.status] || a.status}${
          a.status === 'declined' && a.decline_reason ? ' · ' + a.decline_reason : ''}</div>
      </div>
      <div style="text-align:right">
        ${a.status === 'approved' && a.paid && a.pay_cents ? `<div><strong>${euro(a.pay_cents)}</strong></div>` : ''}
        ${wegKnopf}
      </div>
    </div>`;
  };
  ...
  return `<div class="card">
    <h2>Urlaub</h2>
    <div class="row spread">
      <strong>${k.rest} von ${k.anspruch} Tagen frei</strong>
      <span class="small muted">${k.jahr}${k.genommen ? ' · ' + k.genommen + ' genommen' : ''}</span>
    </div>
    ${k.uebertrag_rest
      ? `<div class="small muted">Davon ${k.uebertrag_rest} aus dem Vorjahr — die verfallen am ${fmtTag(k.uebertrag_verfaellt)}.</div>`
      : ''}
    ${k.uebertrag_unbekannt
      ? `<div class="small muted">Resturlaub aus der Zeit vor Putzflow ist hier nicht
         eingerechnet — bitte bei Ihrer Chefin nachfragen.</div>`
      : ''}
    ${u.antraege.length ? u.antraege.map(zeile).join('') : ''}
    <form id="neuerurlaub" style="margin-top:.6rem">
      <div class="row">
        <label class="small muted" style="flex:1;min-width:8rem">Von
          <input name="von" type="date" required style="width:100%"></label>
        <label class="small muted" style="flex:1;min-width:8rem">Bis
          <input name="bis" type="date" style="width:100%"></label>
      </div>
      <input name="note" placeholder="Notiz (freiwillig)" style="width:100%;margin-top:.4rem">
      <button class="primary" type="submit" style="margin-top:.4rem">Urlaub beantragen</button>
      <p class="small muted" style="margin:.3rem 0 0">Bitte den ganzen Zeitraum angeben,
      auch mit Wochenende. Gezählt werden nur ${u.werktage === 5 ? 'Montag bis Freitag' : 'Werktage (Mo–Sa)'},
      Sonn- und Feiertage gehen nie vom Urlaub ab.</p>
    </form>
  </div>`;
}
...
document.addEventListener('submit', async ev => {
  const u = ev.target.closest('#neuerurlaub');
  if (u) {
    ev.preventDefault();
    const d = new FormData(u);
    try {
      const r = await api('/urlaub', { von: d.get('von'), bis: d.get('bis') || d.get('von'), note: d.get('note') });
      toast(`Beantragt — ${r.tage} ${r.tage === 1 ? 'Tag' : 'Tage'}. Ihre Chefin bekommt Bescheid.`);
      await load();
...
document.addEventListener('click', async ev => {
  ...
  if (urlWeg) {
    if (!confirm('Diesen Urlaub zurückziehen?')) return;
    try { await api(`/urlaub/${urlWeg.dataset.urlWeg}`, null, 'DELETE'); await load(); toast('Zurückgezogen.'); }
    catch (e) { toast(e.message); }
    return;
  }
