...
app.delete('/api/smoobu', requireTenant, auth.requireAdmin, (req, res) => {
  run(`UPDATE tenants SET smoobu_key = NULL, smoobu_secret = NULL WHERE id = ?`, req.tenant.id);
  res.json({ ok: true });
});
...
// Stündlicher Abgleich für alle Mandanten mit hinterlegtem Zugang.
...
async function rundrufStarten(tenant, job, { ausser = [], anlass = '' } = {}) {
  const { gefragt, uebersprungen } = rundruf.kandidaten(tenant, job, ausser);
...
bootstrap();
setInterval(() => { smoobuTick().catch(e => console.error('[smoobu]', e.message)); }, 60 * 60 * 1000).unref();
