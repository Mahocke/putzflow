...
// ausserIds: Personen, die gerade abgesagt haben — die fragt man nicht sofort erneut.
function kandidaten(tenant, job, ausserIds = []) {
  const crew = all(
    `SELECT * FROM users
  ...
  for (const u of crew) {
    if (ausserIds.includes(u.id)) { uebersprungen.push({ user: u, grund: 'hat gerade abgesagt' }); continue; }
