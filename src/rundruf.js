...
function kandidaten(tenant, job, ausserIds = [], ausserGrund = 'hat gerade abgesagt') {
  ...
  for (const u of crew) {
    ...
    // ⚠️ Wer an dem Tag genehmigten Urlaub hat, wird GAR NICHT erst gefragt —
    // aus demselben Grund wie bei der Verdienstgrenze: Eine Anfrage, die eine
    // Absage erzwingt, ist keine Anfrage, sondern eine Zumutung. Der Betrieb hat
    // den Urlaub selbst genehmigt; ihn zwei Wochen später zu ignorieren, ist der
    // Fehler, den ein Urlaubsplan verhindern soll.
    // Sie steht trotzdem in der Mail an die Verwaltung: „drei von sieben gefragt"
    // ist ohne Grund nicht von einem Fehler zu unterscheiden.
    if (urlaub.imUrlaub(u.id, job.due_date)) {
      uebersprungen.push({ user: u, grund: 'hat an dem Tag Urlaub' });
      continue;
    }
