...
function init() {
  ...
  db.exec(`
    ...
    -- Urlaubsanträge. Die Kraft beantragt über ihren Magic-Link, die Verwaltung
    -- genehmigt in der Oberfläche oder direkt aus der Mail heraus.
    --
    -- ⚠️ Tage, Satz, Betrag und Zähleinheit werden BEI DER GENEHMIGUNG eingefroren.
    -- Der 13-Wochen-Schnitt aus § 11 BUrlG bewegt sich mit jedem neuen Auftrag; ohne
    -- Einfrieren änderte sich ein längst abgezeichneter Stundenzettel Wochen später
    -- von selbst — und mit ihm der Signatur-Hash.
    CREATE TABLE IF NOT EXISTS vacation_requests (
      id                INTEGER PRIMARY KEY AUTOINCREMENT,
      tenant_id         INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
      user_id           INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      start_date        TEXT NOT NULL,                    -- YYYY-MM-DD, erster Urlaubstag
      end_date          TEXT NOT NULL,                    -- YYYY-MM-DD, letzter Urlaubstag (inkl.)
      note              TEXT,                             -- freiwillige Notiz der Kraft
      status            TEXT NOT NULL DEFAULT 'pending',  -- pending | approved | declined | cancelled
      days              REAL,                             -- gezählte Urlaubstage (Snapshot)
      werktage          INTEGER,                          -- Zähleinheit bei der Genehmigung (5 oder 6)
      paid              INTEGER NOT NULL DEFAULT 1,       -- Snapshot users.vacation_paid
      pay_per_day_cents INTEGER,                          -- Urlaubsentgelt je Tag (Snapshot)
      pay_cents         INTEGER,                          -- days × pay_per_day_cents
      decided_at        TEXT,
      decided_by        INTEGER REFERENCES users(id) ON DELETE SET NULL,
      decided_via       TEXT,                             -- app | mail
      decline_reason    TEXT,
      requested_at      TEXT NOT NULL DEFAULT (datetime('now'))
    );

    -- Genehmigen/Ablehnen direkt aus der Mail. Eigene Tabelle statt einer Spalte an
    -- vacation_requests: Ein Antrag hat ZWEI Tokens mit verschiedener Wirkung, und
    -- jedes muss einzeln als verbraucht markierbar sein.
    CREATE TABLE IF NOT EXISTS vacation_action_tokens (
      token       TEXT PRIMARY KEY,                       -- sha256(raw), nie das Token selbst
      tenant_id   INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
      request_id  INTEGER NOT NULL REFERENCES vacation_requests(id) ON DELETE CASCADE,
      action      TEXT NOT NULL,                          -- approve | decline
      used_at     TEXT,
      created_at  TEXT NOT NULL DEFAULT (datetime('now'))
    );
    ...
    CREATE INDEX        IF NOT EXISTS idx_vac_user      ON vacation_requests(user_id, status);
    CREATE INDEX        IF NOT EXISTS idx_vac_range     ON vacation_requests(tenant_id, start_date, end_date);
  `);
  ...
  // --- Urlaub ---------------------------------------------------------------
  // ⚠️ KEIN Standardwert bei `vacation_days`. NULL heißt „für diese Person wird kein
  // Urlaubskonto geführt" — und das ist der richtige Ausgangszustand, nicht 20 oder
  // 24. Der Anspruch steht im Arbeitsvertrag und schwankt (Vollzeit, Minijob mit zwei
  // Einsätzen die Woche, eine Reinigungsfirma als Auftragnehmerin ohne jeden
  // Anspruch). Dieselbe Regel wie beim Bundesland: Bei Feldern, aus denen sich Geld
  // ableitet, nie vorbelegen — eine plausibel aussehende falsche Zahl fällt niemandem
  // auf, bis der Urlaub falsch abgerechnet ist.
  addColumn('users', 'vacation_days', `REAL`);
  // Ab wann der Anspruch läuft (Eintritt oder Vertragsumstellung). Leer = schon immer.
  // Steuert § 5 BUrlG (Zwölftelung) UND ob die Urlaubsseite der Kraft überhaupt
  // etwas zeigt.
  addColumn('users', 'vacation_start', `TEXT`);
  // Bezahlter Urlaub ist der gesetzliche Normalfall (§ 1 BUrlG). Die Ausnahme gibt es
  // trotzdem: unbezahlte Freistellung auf Wunsch, oder eine beauftragte Firma, die
  // nur „an diesen Tagen nicht" meldet. Dann blockiert der Eintrag die Planung, ohne
  // Entgelt auszulösen.
  addColumn('users', 'vacation_paid', `INTEGER NOT NULL DEFAULT 1`);
  // ⚠️ Ab wann Putzflow das Konto FÜHRT — nicht zu verwechseln mit `vacation_start`
  // (ab wann der Anspruch besteht). Der Unterschied entscheidet über den Übertrag:
  // Für Jahre vor der Einführung gibt es hier keine Anträge, und aus „keine Anträge"
  // wäre sonst „voller Anspruch nicht genommen" geworden. Wird beim erstmaligen
  // Eintragen der Urlaubstage gesetzt, siehe src/urlaub.js → konto().
  addColumn('users', 'vacation_tracked_since', `TEXT`);
  // Zähleinheit des Mandanten: 6 = Werktage Mo–Sa (§ 3 Abs. 2 BUrlG, der gesetzliche
  // Begriff), 5 = Arbeitstage Mo–Fr. Hier IST ein Default richtig, anders als oben:
  // Es ist keine geschätzte Zahl, sondern die Einheit, die das Gesetz selbst benutzt.
  // ⚠️ Sie muss in Anspruch, Verbrauch und Entgelt dieselbe sein — siehe src/urlaub.js.
  addColumn('tenants', 'urlaub_werktage', `INTEGER NOT NULL DEFAULT 6`);
  ...
  // ⚠️ Bestandsdatenbanken hatten auf `region` noch `NOT NULL DEFAULT 'NW'`. Das
  // war vom 26.07. bis zum 21.08.2026 der Grund, warum sich auf putzflow.de
  // NIEMAND anmelden konnte: Seither gibt der INSERT ausdrücklich NULL mit, damit
  // nicht still NRW angenommen wird — und genau das verletzt die alte Bedingung.
  //
  // Hier stand vorher, ein Tabellen-Neubau sei „für einen Standardwert
  // unverhältnismäßig". Er war die einzige Reparatur. Die Rechnung dafür hat ein
  // Interessent bezahlt, der es siebenmal versucht hat und dann geschrieben hat,
  // die Registrierung sei „derzeit nicht möglich". Merksatz für das nächste Mal:
  // Wer im Migrationskommentar begründet, warum eine Bedingung stehen bleiben
  // darf, hat den Fall schon in der Hand — dann auch zu Ende bringen.
  notNullLoesen('tenants', 'region');
  // Bestandswerte bleiben unangetastet: Ob 'NW' gewählt oder nur gefallen ist,
  // lässt sich nachträglich nicht unterscheiden.
...
// Eine NOT-NULL-Bedingung von einer Spalte nehmen. SQLite kann das nicht per
// ALTER TABLE, also der offizielle Weg über eine neue Tabelle
// (sqlite.org/lang_altertable.html#otheralter).
//
// ⚠️ Die neue Tabelle entsteht aus dem GELEBTEN Schema (`sqlite_master`), nicht aus
// dem `CREATE TABLE` oben. Baute man sie aus dem Quelltext nach, verlöre eine
// Bestandsdatenbank alle Spalten, die seither per `addColumn` dazugekommen sind —
// bei `tenants` sind das über zwanzig.
//
// ⚠️ Und der Umbau wird VOR dem Commit gegengeprüft: Spaltenzahl, Reihenfolge,
// Namen, Typen und Zeilenzahl müssen übereinstimmen, nur die eine Bedingung darf
// sich geändert haben. Ein Tabellen-Neubau, der still eine Spalte verliert, wäre
// schlimmer als der Fehler, den er behebt.
function notNullLoesen(tabelle, spalte) {
  const vorher = all(`PRAGMA table_info(${tabelle})`);
  const info = vorher.find(c => c.name === spalte);
  if (!info || !info.notnull) return;          // schon frei — idempotent

  const zeile = get(`SELECT sql FROM sqlite_master WHERE type='table' AND name = ?`, tabelle);
  if (!zeile || !zeile.sql) throw new Error(`${tabelle}: kein Schema gefunden`);

  // Die Spaltendefinition: der Name als eigenständiges Wort, danach der Typ, dann
  // optional NOT NULL und optional ein Standardwert. Beides fällt weg — der
  // Standardwert 'NW' ist genau der stille Rückfall, der hier nicht mehr gelten soll.
  // `DEFAULT (datetime('now'))` kommt in derselben Tabelle vor, deshalb die
  // Klammer-Variante ausdrücklich mit im Muster.
  ...
  if (!muster.test(zeile.sql)) throw new Error(`${tabelle}.${spalte}: Definition nicht erkannt`);
  const neuesSchema = zeile.sql.replace(muster, '$1$2')
    .replace(new RegExp(`^CREATE TABLE\\s+"?${tabelle}"?`, 'i'), `CREATE TABLE ${tabelle}__neu`);

  // Nebentabellen zeigen per FOREIGN KEY hierher; ohne das Ausschalten scheitert
  // schon das DROP. Muss außerhalb der Transaktion stehen.
  const fkVorher = get(`PRAGMA foreign_keys`).foreign_keys;
  db.exec(`PRAGMA foreign_keys = OFF`);
  db.exec(`BEGIN`);
  try {
    const zeilenVorher = get(`SELECT COUNT(*) AS n FROM ${tabelle}`).n;
    db.exec(neuesSchema);
    db.exec(`INSERT INTO ${tabelle}__neu SELECT * FROM ${tabelle}`);

    const nachher = all(`PRAGMA table_info(${tabelle}__neu)`);
    const zeilenNachher = get(`SELECT COUNT(*) AS n FROM ${tabelle}__neu`).n;
    if (zeilenNachher !== zeilenVorher) {
      throw new Error(`${zeilenVorher} Zeilen rein, ${zeilenNachher} raus`);
    }
    if (nachher.length !== vorher.length) {
      throw new Error(`${vorher.length} Spalten vorher, ${nachher.length} nachher`);
    }
    for (let i = 0; i < vorher.length; i++) {
      const a = vorher[i], b = nachher[i];
      if (a.name !== b.name || a.type !== b.type || a.pk !== b.pk) {
        throw new Error(`Spalte ${i + 1} weicht ab: ${a.name} ${a.type} → ${b.name} ${b.type}`);
      }
      const sollNotNull = a.name === spalte ? 0 : a.notnull;
      if (b.notnull !== sollNotNull) throw new Error(`${a.name}: NOT NULL stimmt nicht`);
    }

    db.exec(`DROP TABLE ${tabelle}`);
    db.exec(`ALTER TABLE ${tabelle}__neu RENAME TO ${tabelle}`);
    // Indizes und Trigger gehen mit dem DROP verloren. Bei `tenants` gibt es heute
    // nur den impliziten Index aus `slug … UNIQUE`, der im Schematext steht und
    // damit von selbst wieder entsteht — aber darauf soll sich die nächste
    // Tabelle nicht verlassen müssen.
    for (const o of all(
    ...
    db.exec(`ROLLBACK`);
    db.exec(`PRAGMA foreign_keys = ${fkVorher ? 'ON' : 'OFF'}`);
    throw new Error(`${tabelle}.${spalte}: Umbau abgebrochen, nichts geändert — ${e.message}`);
  }
  db.exec(`PRAGMA foreign_keys = ${fkVorher ? 'ON' : 'OFF'}`);

  const verwaist = all(`PRAGMA foreign_key_check`);
  if (verwaist.length) throw new Error(`${tabelle}: ${verwaist.length} verwaiste Fremdschlüssel nach dem Umbau`);
  console.log(`[db] ${tabelle}.${spalte}: NOT NULL entfernt`);
}
