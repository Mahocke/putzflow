...
// ⚠️ `heute` ist ein Parameter, kein `new Date()` mitten im Rumpf. Beim Urlaub
// entscheidet der heutige Tag darüber, was schon genommen und was noch geplant ist —
// eine Rechnung, die sich still an der Systemuhr bedient, lässt sich nicht prüfen
// und ändert ihr Ergebnis über Nacht. Im Betrieb bleibt der Standardwert.
function timesheet(tenant, userId, dateStr, heute = new Date().toISOString().slice(0, 10)) {
  ...
  // Genehmigter Urlaub in dieser Periode.
  // ⚠️ Urlaubsentgelt ist ARBEITSENTGELT (§ 11 BUrlG, § 14 SGB IV): Es zählt in die
  // Minijob-Grenze und gehört auf den Zettel für die Lohnbuchhaltung. Es ist aber
  // KEINE ARBEITSZEIT — deshalb steht es in einer eigenen Liste und nicht in `items`.
  // Landete es dort, ginge es als „Position ohne erfasste Zeit" in die
  // Mindestlohn-Prüfung: einmal als Geld ohne Stunden, das den Schnitt hebt und einen
  // Verstoß verdeckt, und einmal als gezählter Mangel („nicht prüfbar"), obwohl an
  // einem Urlaubstag völlig zu Recht keine Zeit aufgezeichnet ist.
  // ⚠️ `require` im Rumpf: siehe Kopf von src/urlaub.js (Ring zwischen beiden Modulen).
  const urlaub = require('./urlaub').fuerPeriode(tenant, userId, period, heute);

  // cents/minutes enthalten nur Geleistetes — Geplantes steht getrennt in geplant_cents.
  const entgeltCents = cents + auslagen.entgelt_cents + urlaub.entgelt_cents;
  const entgeltMinuten = minutes + auslagen.minuten;
  geplantCents += urlaub.geplant_cents;
  ...
  // Der Urlaub geht mit in den Hash: Er steht als Betrag auf dem Zettel, also muss
  // eine nachträgliche Änderung — Antrag zurückgezogen, Zeitraum korrigiert — die
  // Unterschrift veralten lassen. Nur bereits genommene Tage; künftige kann niemand
  // bestätigen, und sie halten den Hash stabil, während die Planung noch läuft.
...
// --- Absagen und wieder einplanen ---------------------------------------
//
// Die Reinigung fällt weg — weil der Gast nicht kam, weil die Wohnung doch nicht
// gebraucht wird. Das ist etwas ANDERES als „diese Kraft kann nicht": dafür gibt
// es das Umteilen und die „Zuteilung entfernen"; der Termin bleibt dabei stehen
// und will neu vergeben werden.
//
// ⚠️ Die Zuteilung wird beim Absagen GELEERT, samt Zeitschlitz und Anfrage-Merker.
// Die Kraft hat in diesem Moment eine Absage bekommen (Mail, und wenn sie zugesagt
// hatte, auch die Kalender-Absage). Bliebe sie zugeteilt, stünde ein später wieder
// eingeplanter Termin als „zugesagt" da — auf eine Zusage gestützt, die für eine
// abgesagte Reinigung gegeben wurde. Nach dem Wiedereinplanen wird deshalb neu
// gefragt, genau wie bei einer verschobenen Buchung in `sync.js`.
//
// Der Status bleibt `skipped`, kein neuer fünfter Zustand: Stundenzettel, Lohn und
// Minijob-Grenze zählen längst nur `open` und `done` (siehe `timesheet`), und die
// Kalenderabfragen der Kräfte ebenso. Ein eigener Status hieße, jede dieser Stellen
// noch einmal zu prüfen — für eine Unterscheidung, die nur `skipped_by` braucht.
function absagen(job) {
  run(`UPDATE jobs SET status = 'skipped', skipped_by = 'admin', assigned_user_id = NULL,
                       confirmed = 0, requested_at = NULL, start_time = NULL
        WHERE id = ?`, job.id);
}

function wiederEinplanen(job) {
  run(`UPDATE jobs SET status = 'open', skipped_by = NULL WHERE id = ?`, job.id);
}

// Die Kraft fällt aus — die Reinigung bleibt. Der Gegenfall zum Absagen, und der
// häufigere: krank, Auto kaputt, Kind krank.
//
// ⚠️ `declined_at` wird NICHT gesetzt. Das ist der Merker dafür, dass die KRAFT
// selbst abgesagt hat; hier meldet die Verwaltung sie ab. Stünde er, zeigte die
// Karte „abgesagt", und in der Kandidatenliste des nächsten Rundrufs stünde bei
// ihr „hat gerade abgesagt" — beides über sie behauptet, ohne dass sie etwas
// gesagt hat.
function zuteilungFreigeben(job) {
  run(`UPDATE jobs SET assigned_user_id = NULL, confirmed = 0, requested_at = NULL,
                       start_time = NULL
        WHERE id = ?`, job.id);
}
...
module.exports = { resolveRule, sessionsFor, jobPay, timesheet, AUFGABE,
                   ...
                   absagen, wiederEinplanen, zuteilungFreigeben };
